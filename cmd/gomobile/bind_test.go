...
func TestBindAndroid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		cmdBind.flag.Parse([]string{"github.com/mixinnetwork/mobile/asset"})
		err := runBind(cmdBind)
		if err != nil {
			t.Log(buf.String())
			t.Fatal(err)
...
import (
	_ "github.com/mixinnetwork/mobile/app"
)
...
func TestBindWithGoModules(t *testing.T) {
	...
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/mixinnetwork/mobile/cmd/gobind").CombinedOutput(); err != nil {
	...
	for _, target := range []string{"android", "ios"} {
		...
		t.Run(target, func(t *testing.T) {
			...
			tests := []struct {
				...
				{
					Name: "Absolute Path",
					Path: "github.com/mixinnetwork/mobile/bind/testdata/cgopkg",
				},
				{
					Name: "Relative Path",
