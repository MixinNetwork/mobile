...
func runVersion(cmd *command) (err error) {
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/mixinnetwork/mobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/mixinnetwork/mobile/app").CombinedOutput()
	if err != nil {
